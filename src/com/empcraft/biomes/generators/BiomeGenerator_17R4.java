 ...
 */
public class BiomeGenerator_17R4 extends BiomeGenerator {
    ...
    public BiomeGenerator_17R4(final Biome biome, long seed) {
        this.biome = biome;
        this.seed = seed;
    ...
    public boolean generate(final BiomeSelection selection, final Player player, final boolean sameProvider, final Runnable whenDone) {
        ...
        for (int xv = selectionArray[0]; xv <= selectionArray[2]; xv += 16) {
            for (int zv = selectionArray[1]; zv <= selectionArray[3]; zv += 16) {
                ...
                final long index = BiomeHandler.counter;
                ...
                Bukkit.getScheduler().runTaskLater(plugin, new Runnable() {
                    ...
                    public void run() {
                        if ((index != 0) && ((index % 10) == 0)) {
                            Main.sendMessage(Bukkit.getPlayer(BiomeHandler.runner), "&7 - chunk &6" + index + "&7/&6" + chunks);
                        }
                        final Block[] chunk = createChunk(generator, world, x >> 4, z >> 4, BiomeGenerator_17R4.this.biomeBase);
                        for (int cx = 0; cx < 16; ++cx) {
                            for (int cz = 0; cz < 16; ++cz) {
                                ...
                                if ((wx >= selectionArray[0]) && (wz >= selectionArray[1]) && (wx <= selectionArray[2]) && (wz <= selectionArray[3])) {
                                    craftWorld.setBiome(wx, wz, BiomeGenerator_17R4.this.biome);
                ...
                try {
                    ...
                    for (int x = selectionArray[0] + Main.inset; x <= (selectionArray[2] - Main.inset); x += 16) {
                        for (int z = selectionArray[1] + Main.inset; z <= (selectionArray[3] - Main.inset); z += 16) {
                            ...
                            try {
                                BiomeGenerator_17R4.this.biomeBase.a(w2, ReflectionUtils.getFieldValue(ChunkProviderGenerate.class, "i", Random.class, generator), (x >> 4) * 16, (z >> 4) * 16);
                            } catch (final NoSuchFieldException e) {
                                e.printStackTrace();
                            }
                        }
